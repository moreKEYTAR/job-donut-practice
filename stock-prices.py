...
def get_max_profit(stock_prices):
    ...
    left_buy = stock_prices[0]
    max_profit = stock_prices[1] - stock_prices[0]

    for i in xrange(2, len(stock_prices)):

        if stock_prices[i - 1] < left_buy:
            left_buy = stock_prices[i - 1]

        if stock_prices[i] - left_buy > max_profit:
            max_profit = stock_prices[i] - left_buy
